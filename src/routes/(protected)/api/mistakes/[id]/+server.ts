...
export const PATCH: RequestHandler = async ({ params, request, locals: { supabase, safeGetSession } }) => {
	...
	if (!data) return json({ success: false, message: "Mistake not found" }, { status: 404 });

	return json({ success: true, data, message: "Mistake updated successfully." });
};
...
export const DELETE: RequestHandler = async ({ params, locals: { supabase, safeGetSession } }) => {
	...
	return json({ success: true, message: "Mistake deleted successfully." });
};
